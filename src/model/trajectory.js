...
TRAFFIC.Trajectory.prototype = {
    ...
    canEnterIntersection : function() {
        ...
        //console.log("can_enter:"+turnNumber);
        var s=TRAFFIC.settings.signals;
        return ((s==false)?true: intersection.controlSignals.state[sideId][turnNumber]);
    },
    getDistanceToIntersection : function() {
        var distance;
        distance = this.current.lane.length - this.car.length / 2 - this.current.position;
        //console.log(distance);
        if(distance<25) if(this.car.speed>this.car.maxSpeed/2) this.car.speed=this.car.speed*0.99;
    ...
    moveForward : function(distance) {
        ...
        if(this.temp.lane !=null && this.temp.lane instanceof TRAFFIC.Curve) {
            switch(this.car.nextDirection) {
                case 2: this.car.inCurve=2; break;
                case 0: this.car.inCurve=0; break;
                default: this.car.inCurve=1; break;
            }
        } else this.car.inCurve=-1;
        //console.log("temp.lane:")
    ...
    changeLane : function(nextLane) {
        ...
        //JLA nextPosition = this.current.position + 3 * this.car.length;
    ...
    _startChangingLanes : function(nextLane, nextPosition) {
       ...
       //}
        ...
        if(this.car.speed>20) {
            this.car.speed-=15; //JLA ???????????????????
            //console.log(".");
        } else if(this.car.speed>10) {
            this.car.speed-=6; //JLA ???????????????????
            //console.log(".");
        } else if(this.car.speed>5) {
            this.car.speed-=2; //JLA ???????????????????
            //console.log(".");
       }
       */
