...
TRAFFIC.World.prototype = {
    ...
    load : function() {
        ...
        for (id in _ref) {
            ...
            var isect=new TRAFFIC.Intersection();
            this.addIntersection(isect.copy(intersection));
        }
        _ref1 = data.roads;
        _results = [];
        for (id in _ref1) {
    ...
    //generateMap : function(minX, maxX, minY, maxY, Linemax, mult) {
        ...
        for(y= minY;y<=maxY;y++) { 
            ...
            for(x= minX;x<=maxX;x++) {
                ...
                if(y==minY || y==maxY|| x==minX || x==maxX || TRAFFIC.random() < 0.5) {
                    if (previous != null) this.addRoad(new TRAFFIC.Road(intersection, previous));
                    if (previous != null) this.addRoad(new TRAFFIC.Road(previous, intersection));
                    } else {
                        var debug=true;
